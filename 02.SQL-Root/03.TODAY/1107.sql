...
-- -> %를 중간에 사용하여 어떤로 시작하고 어떤로 끝나는 모든것 검색
-- 쿼리11 : `customers`테이블
...
-- 데이터가 나왔으니 null값도 제외됨(구체적인 값이 없으므로 자동제외)
...
-- 쿼리16: `orders` 테이블
SELECT * FROM `orders` WHERE `payment_type` = "Check" AND NOT `ship_city` = "New York";
...
SELECT COUNT(*) AS "전체 레코드수" FROM `orders` WHERE `payment_type` = "check" AND NOT `ship_city` = "new york" OR `ship_name` = "Christina Lee";