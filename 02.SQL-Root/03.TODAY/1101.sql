...
-- title 컬럼의 데이터형을 기존 varchar(200)에서 varchar(300)으로 변경
ALTER TABLE `todolist`
...
VALUES ("SQL 데이터빼기", "insert int로 데이터를 빼보자","A");

-- 문자데이터항목에 숫자로 넣어도 문자형 숫자로 입력됨
INSERT INTO `todolist` (`title`,`cont`,`auth`)
VALUES (333, "insert int로 데이터를 빼보자","A");

-- auth항목의 한글자 데이터인데 두글자 넣은 경우
INSERT INTO `todolist` (`title`,`cont`,`auth`)
VALUES ("SQL 데이터빼기", "insert int로 데이터를 빼보자","AA");
-- 입력성공 하단에 경고메시지 나옴
-- 메시지:
